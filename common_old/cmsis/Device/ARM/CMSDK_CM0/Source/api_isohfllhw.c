...
uint32_t _isohf_ctrlISOADESELECT_local(HFCTRL isohf, uint8_t cid_exist, uint8_t cid, uint32_t frame_size, uint32_t silent_time)
{
  ...
  // note : power level indication is set to (00)b
    ...
    deselect_nocid= 0xC2;  // frame size 1 (out of CRC)
          ...
          // check CID & frame_size
             //-------------------------------------
          if ((cid_exist == 1) && (_isohf_getHFIORAMbyte_local(1) == (uint8_t) cid) && (frame_size==4)){ // CID with power level indication 00
         ...
         else if((cid_exist == 0) && (frame_size==3)){ // NO CID expected                                                        
...
uint8_t isohf_GetBlock_Type(void) {
   uint8_t PCB = _isohf_getHFIORAMbyte_local(0);
   return ((PCB & IRS_BLOCK_MASK) >> IRS_BLOCK_SHIFT);
}
...
//   *i_block    	      =  (PCB & IRS_BLOCK_MASK) >> IRS_BLOCK_SHIFT;
   *chaining_bit_en   =  (PCB & I_CHANINIG_MASK) >>  I_CHANINIG_SHIFT;
   *cid_exist         =  (PCB & CID_MASK) >> CID_SHIFT;
   *block_num         =  (PCB & BLOCK_NUMBER_MASK) >> BLOCK_NUMBER_SHIFT;
   if(((PCB & (1 << 1)) == 0x02) && ((PCB & (1 << 5)) == 0x0)) {
      *error = 0x0;
   }
   else {
      *error = 0x1;
...
//   *r_block    	      =  (PCB & IRS_BLOCK_MASK) >> IRS_BLOCK_SHIFT;
   *ACK               =  (PCB & R_ACK_OR_NAK_MASK) >> R_ACK_OR_NAK_SHIFT;
   *cid_exist         =  (PCB & CID_MASK) >> CID_SHIFT;
   *block_num         =  (PCB & BLOCK_NUMBER_MASK) >> BLOCK_NUMBER_SHIFT;
   if(((PCB & (1 << 1)) == 0x02) && ((PCB & (1 << 2)) == 0x0) && ((PCB & (1 << 5)) == 0x20)) {
      *error = 0x0;
   }
   else {
      *error = 0x1;
...
//   *s_block    	      =  (PCB & IRS_BLOCK_MASK) >> IRS_BLOCK_SHIFT;
   *FUNC_SEL          =  (PCB & S_FUNC_SEL_MASK) >> S_FUNC_SEL_SHIFT;
   *FUNC              =  (PCB & S_FUNC_MASK) >> S_FUNC_SHIFT;
   *cid_exist         =  (PCB & CID_MASK) >> CID_SHIFT;
   if(((PCB & (0x01)) == 0x0) && ((PCB & (1 << 2)) == 0x0)) {
      *error = 0x0;
   }
   else {
      *error = 0x1;
...
{  
   ...
   if(block_num == 1 )      PCB |= BLOCK_NUMBER_MASK; 
                            PCB |= 1 << 1;
   if(cid_exist == 1 )      PCB |= CID_MASK;
   if(chaining_bit_en == 1 )PCB |= I_CHANINIG_MASK;
                            PCB |= I_BLOCK_MODE << IRS_BLOCK_SHIFT ;
  // Copy I block(PCB/CID) to IORAM BUFFER
   ((uint8_t *)(PLATFORM_HF_BUFFER_ADDR))[0]   = PCB;  //PCB
   if(cid_exist) {
...
uint32_t isohf_SetRblock(uint8_t ACK, uint8_t cid_exist, uint8_t cid, uint8_t block_num)
{
  ...
  if(block_num == 1 )  PCB |= BLOCK_NUMBER_MASK;      	         
                       PCB |= 1 << 1;                               	        
  if(cid_exist == 1 )  PCB |= CID_MASK;                             	        
  if(ACK == 1 )        PCB |= R_ACK_OR_NAK_MASK ;  	  // send pos/neg Acknowledgment            
                       PCB |= 1 << 5;
                       PCB |= R_BLOCK_MODE << IRS_BLOCK_SHIFT ;
